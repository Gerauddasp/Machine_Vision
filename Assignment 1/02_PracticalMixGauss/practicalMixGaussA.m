...
for (cY = 1:imY); 
    ...
    for (cX = 1:imX);          
        ...
        posteriorSkin(cY,cX) = (likeSkin * priorSkin)/(likeSkin * priorSkin + likeNonSkin * (1 - priorSkin)) ;
    end;
end;
...
SigmaInv = inv(gaussCov);
GaussianProb = (1/(sqrt(((2*pi).^3)*SigmaDet)))*exp(-0.5*(data-gaussMean)'*SigmaInv*(data-gaussMean));
...
XminMu = data - repmat(meanData,1,nData);
covData = (XminMu * XminMu')/(nData - 1);
