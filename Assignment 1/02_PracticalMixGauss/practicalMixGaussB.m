...
for (cData =1:nData)
    ...
    if h == 1
        data(1,cData) = mixGauss.mean(1) + mixGauss.cov(1) * randn;
    else
        data(1,cData) = mixGauss.mean(2) + mixGauss.cov(2) * randn;
    end
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        %replace this: 
        thisData = data(:,cData); 
        like1 = mixGaussEst.weight(1)*getGaussProb(thisData,mixGaussEst.mean(1),mixGaussEst.cov(1)); 
        like2 = mixGaussEst.weight(2)*getGaussProb(thisData,mixGaussEst.mean(2),mixGaussEst.cov(2));
        like = like1 + like2;
        responsibilities(1,cData) = like1 / like;
        responsibilities(2,cData) = like2 / like;
   end;
   ...
   for (cGauss = 1:k) 
        ...
        mixGaussEst.cov(1,1,cGauss) = responsibilities(cGauss,:).*(data - mixGaussEst.mean(cGauss))*(data-mixGaussEst.mean(cGauss))'/sum(responsibilities(cGauss,:),2);
   end;
...
for(cData = 1:nData)
    ...
    like1 = mixGaussEst.weight(1)*getGaussProb(thisData,mixGaussEst.mean(1),mixGaussEst.cov(1)); 
    like2 = mixGaussEst.weight(2)*getGaussProb(thisData,mixGaussEst.mean(2),mixGaussEst.cov(2));    
    boundValue1 = thisQ(1) * log(like1/thisQ(1));
    boundValue2 = thisQ(2) * log(like2/thisQ(2));
