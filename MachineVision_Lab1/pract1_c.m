...
betaP = sum(X)/2+ beta + (gamma*delta.^2)/2 - (gamma*delta + sum(X))/(2*(gamma+N));
 
 
%% ML + MAP Parameter Estimation of univariate normal
% TODO - fill these equations from the previous section
muML = sum(X)/N;
sigmaML = sqrt(sum((X-mean(X)).^2)/N);
muMAP = (sum(X)+gamma*delta)/(N+gamma) ;
sigmaMAP = ((2*beta + gamma*((delta-mu).^2))*sum((X-mu).^2))/(N+3+2*alpha);
